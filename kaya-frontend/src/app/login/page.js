  ...
  const handleAddToBag = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Syncs the cart badge in the Navbar
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className={`fixed top-24 right-6 z-50 transform transition-all duration-300 ${showToast ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
        <div className="bg-emerald-600 text-white px-6 py-4 rounded-xl shadow-2xl shadow-emerald-900/50 flex items-center gap-3 border border-white/10">
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 items-start">
          ...
          <div className="w-full lg:w-[40%] space-y-4 lg:sticky lg:top-28">
            <div className="relative overflow-hidden bg-[#111] border border-white/5 aspect-[4/5] rounded-2xl group cursor-zoom-in">
              {product.images && product.images[selectedImage] ? (
                ...
                <div className="flex items-center justify-center h-full text-gray-600 italic">No Image Available</div>
              )}
            </div>
          ...
          <div className="w-full lg:w-[60%] flex flex-col">
            ...
            <p className="text-gray-400 text-sm leading-relaxed mb-10 font-light border-l-2 border-emerald-500/30 pl-4">
              {product.description || "Premium quality handcrafted product."}
            </p>
            ...
            <div className="flex gap-4 mb-12">
              <button 
                ...
                disabled={adding}
                className="flex-1 bg-white text-black py-4 px-8 font-bold tracking-[0.15em] text-sm uppercase hover:bg-gray-200 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl shadow-xl active:scale-95"
              >
            ...
            <div className="border-t border-gray-800">
              ...
              >
                <p className="text-gray-400 text-sm leading-relaxed font-light">
                  {product.fullDescription || product.description || "Premium handcrafted fabric."}
                </p>
              </Accordion>
              ...
              >
                <ul className="text-gray-400 text-sm space-y-2 font-light">
                  <li>• Free shipping in India on orders above ₹999</li>
                  <li>• 7-day easy return policy</li>
                  <li>• Handcrafted in Kalpi</li>
                </ul>
              </Accordion>
            </div>
          </div>
...
function Accordion({ title, children, isOpen, onClick }) {
  return (
    <div className="border-b border-gray-800">
      <button onClick={onClick} className="w-full py-5 flex justify-between items-center text-left group">
        <span className="text-sm tracking-wider font-medium text-gray-300 group-hover:text-white transition-colors uppercase">{title}</span>
        <svg className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${isOpen ? 'rotate-180 text-emerald-400' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        </svg>
      </button>
      <div className={`overflow-hidden transition-all duration-300 ${isOpen ? 'max-h-60 pb-5' : 'max-h-0'}`}>{children}</div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <span className="text-gray-500 text-xs tracking-widest uppercase animate-pulse">Loading Product Details...</span>
      </div>
    </div>
  );
}
...
function NotFound() {
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center text-white p-4">
      ...
      <p className="text-gray-500 mb-8 text-center max-w-md">The item you are looking for might have been moved or is currently out of stock.</p>
      <Link href="/" className="px-8 py-3 bg-white text-black font-bold uppercase tracking-widest text-xs rounded-full hover:bg-gray-200 transition-colors">Return to Shop</Link>
    </div>
  );
}